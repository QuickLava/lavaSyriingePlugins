...
namespace magicSeries
{
    ...
    void magicSeriesCallback(Fighter* attacker, StageObject* target, float damage)
    {
        ...
        if (mechHub::getPassiveMechanicEnabled(fighterPlayerNo, mechHub::pmid_MAGIC_SERIES))
        {
            ...
            switch (currStatus)
            {
            case Fighter::Status::Attack: case Fighter::Status::Attack_100: case Fighter::Status::Attack_Dash: 
            case Fighter::Status::Attack_S3: case Fighter::Status::Attack_Hi3: case Fighter::Status::Attack_Lw3:
            {
                transitionModule->enableTermGroup(0x4);
                transitionModule->unableTermAll(0x4);
                if (currStatus <= Fighter::Status::Attack_Dash)
                {
                    transitionModule->enableTerm(Fighter::Status::Transition::Term_Cont_Attack_S3, 0x4);
                    transitionModule->enableTerm(Fighter::Status::Transition::Term_Cont_Attack_Hi3, 0x4);
                    transitionModule->enableTerm(Fighter::Status::Transition::Term_Cont_Attack_Lw3, 0x4);
                }
                transitionModule->enableTerm(Fighter::Status::Transition::Term_Cont_Attack_S4_Start, 0x4);
                transitionModule->enableTerm(Fighter::Status::Transition::Term_Cont_Attack_Hi4_Start, 0x4);
                transitionModule->enableTerm(Fighter::Status::Transition::Term_Cont_Attack_Lw4_Start, 0x4);

                OSReport_N("- Enabled Attack Cancels!\n");
            }
            case Fighter::Status::Attack_S4: case Fighter::Status::Attack_Lw4: case Fighter::Status::Attack_Hi4: case Fighter::Status::Attack_Air:
            {
                if (moduleEnum->m_situationModule->getKind() == 0x00)
                {
                    transitionModule->enableTermGroup(Fighter::Status::Transition::Group_Chk_Ground_Special);
                }
                else
                {
