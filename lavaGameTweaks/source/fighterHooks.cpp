...
namespace fighterHooks
{
    ...
    // On TransitionTermEstablish Callbacks
    bool ftCallbackMgr::performTransitionOverrideCallbacks(StageObject* stageObj, soInstanceUnitFullProperty<soTransitionTerm>* transitionIn, u32* actionOut, int* termIDOut)
    {
        bool result = 1;
        if (stageObj->m_taskCategory == gfTask::Category_Fighter)
        {
            // Execute Callbacks
            int transitionTermID = transitionIn->m_id;
            u32 transitionTermTargetAction = transitionIn->m_element.m_targetKind;
            OSReport_N("%sPerforming Transition Override Callbacks: TermID: %d, ActionID: %3X\n", outputTag, transitionTermID, transitionTermTargetAction);
            for (u32 i = 0; result && i < m_currBundleCount; i++)
            {
                TransitionTermEventCB currCallback = m_callbackBundles[i]->m_TransitionOverrideCB;
                if (currCallback != NULL)
                {
                    u32 targetAction = currCallback((Fighter*)stageObj, transitionTermID, transitionTermTargetAction);
                    result = targetAction != 0xFFFFFFFF;
                    if (targetAction != transitionTermTargetAction)
                    {
                        *actionOut = targetAction;
                        break;
                    }
                }
            }
    ...
    bool performTransitionOverrideCallbacksBridge(StageObject* stageObj, soInstanceUnitFullProperty<soTransitionTerm>* transitionIn, u32* actionOut, int* termIDOut)
    {
        return ftCallbackMgr::performTransitionOverrideCallbacks(stageObj, transitionIn, actionOut, termIDOut);
    }
    asm void transitionOverrideHook()
    {
        nofralloc
        mflr r31;                                       // Backup LR in a non-volatile register!
                                                        // Call main function body!
        lwz r3, 0x08(r24);                              // Fetch StageObject* from ModuleAccesser in r24.
        mr r4, r30;                                     // Fetch transitionTerm property from r30.
        mr r5, r25;                                     // \ 
        mr r6, r26;                                     // / Fetch return buffer arguments for overwriting the target action.
        bl performTransitionOverrideCallbacksBridge;    // Call!
        mtlr r31;                                       // Restore LR.
        cmplwi r3, 0x01;                                // Check if we returned 1, indicating that the given Transition is okay.
        beqlr+;                                         // If so, we can return, we're all good.
                                                        // Otherwise, we need to jump back into the loop!
        lmw r4, STACK_ELEMENT_OFFSET(se_Reg04)(r1);     // Restore our register backup...
        lwz r1, 0x00(r1);                               // ... and deallocate our trampoline stack frame.
        lwz r12, 0x04(r1);                              // Then, grab the LR value for the main function, from when we exited checkEstablish.
        mtlr r12;                                       // Copy that into LR...
        blr;                                            // ... then bounce to it!
    }
    ...
    void registerFighterHooks()
    {
        ...
        //SyringeCompat::syInlineHookRel(0x3BEB8, reinterpret_cast<void*>(setPartForDeflect), Modules::SORA_MELEE);

        // Match Start Hook @ 0x80813D24: 0x08 bytes into symbol "start/[ftManager]/ft_manager.o"
        SyringeCompat::syInlineHookRel(0x109310, reinterpret_cast<void*>(ftCallbackMgr::performMeleeOnStartCallbacks), Modules::SORA_MELEE);
        ...
        SyringeCompat::syInlineHookRel(0x109944, reinterpret_cast<void*>(ftCallbackMgr::performOnCreateCallbacks), Modules::SORA_MELEE);

        // General Fighter Start Hook @ 0x80814774: 0x10 bytes into symbol "startFighter/[ftManager]/ft_manager.o"
        SyringeCompat::syInlineHookRel(0x109D60, reinterpret_cast<void*>(ftCallbackMgr::performOnStartCallbacks), Modules::SORA_MELEE);

        // General Fighter Exit Hook @ 0x80814384: 0x14 bytes into symbol "removeEntry/[ftManager]/ft_manager.o"
        SyringeCompat::syInlineHookRel(0x109970, reinterpret_cast<void*>(ftCallbackMgr::performOnRemoveCallbacks), Modules::SORA_MELEE);

        // General Fighter Attack Land @ 0x8081A298: 0x3A4 bytes into symbol "notifyLogEventCollisionHit/[ftManager]/ft_manager_log_eve"
        SyringeCompat::syInlineHookRel(0x10F884, reinterpret_cast<void*>(ftCallbackMgr::performOnAttackCallbacks), Modules::SORA_MELEE);
        ...
        // General Fighter Transition Override Hook @ 0x807B5E00: 0xF8 bytes into symbol "checkEstablish/[soTransitionTermGroup]/so_transition_term"
        SyringeCompat::syInlineHookRel(0xAB3EC, reinterpret_cast<void*>(transitionOverrideHook), Modules::SORA_MELEE);
    }
}
