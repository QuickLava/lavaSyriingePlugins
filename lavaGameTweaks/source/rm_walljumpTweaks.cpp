...
namespace rmWalljumpTweaks
{
            ...
            && ftValueAccesser::getConstantInt(moduleAccesser, ftValueAccesser::Info_Param_Int_Wall_Jump_Type, 0))
        {
                ...
                mechHub::getPassiveMechanicEnabled(fighterPlayerNo, mechHub::pmid_WALLJUMP_FROM_SPECIAL) && !(transitionIntoSpecialFallTemp & playerBit))
            {
                ...
                if (groupVec->size() > 0)
                {
                    ...
                    {
                        if (termVecPtr->at(i).m_element.m_targetKind == Fighter::Status::Fall_Special)
                        {
            ...
            && moduleAccesser->m_enumerationStart->m_statusModule->isEnableTransitionTermGroup(Fighter::Status::Transition::Group_Chk_Air_Wall_Jump))
        {
            ...
            if (fabsf(stickXPos) > 0.15f && (finalDir * stickXPos) < 0.0f)
            {
                ...
                if (controllerModule->getTrigger().m_jump)
                {
                    ...
                    moduleEnum->m_statusModule->changeStatusRequest(Fighter::Status::Wall_Jump, moduleAccesser);
                }
            }
        }
    }
        ...
        if (fighterPlayerNo < fighterHooks::maxFighterCount)
        {
            ...
            if (currSituation == Situation_Air)
            {
                ...
                else
                {
                    ...
                    {
                        // ... enable walljumping.
                        statusModule->enableTransitionTermGroup(Fighter::Status::Transition::Group_Chk_Air_Wall_Jump);
                    }
                    ...
                    else if (mechHub::getPassiveMechanicEnabled(fighterPlayerNo, mechHub::pmid_WALLJUMP_FROM_SPECIAL))
                    {
                        ...
                        if (currStatus > Fighter::Status::Test_Motion && (specialWalljumpTemp & playerBit) == 0)
                        {
    ...
    void onStatusChangeCallback(Fighter* fighterIn)
    {
        ...
        if (fighterPlayerNo < fighterHooks::maxFighterCount)
        {
            ...
            walljumpNativeTemp |= statusModule->isEnableTransitionTermGroup(Fighter::Status::Transition::Group_Chk_Air_Wall_Jump) << fighterPlayerNo;
            ...
            if (currSituation == Situation_Air)
            {
                ...
                if (mechHub::getPassiveMechanicEnabled(fighterPlayerNo, mechHub::pmid_WALLJUMP_FROM_SPECIAL))
                {
                    ...
                    else
                    {
                        ...
                        if (currStatus == Fighter::Status::Fall_Special)
                        {
                ...
                if (mechHub::getPassiveMechanicEnabled(fighterPlayerNo, mechHub::pmid_WALLJUMP_SAME_DIR_RESTRICTION))
                {
                    ...
                    if (currStatus == Fighter::Status::Wall_Jump)
                    {
